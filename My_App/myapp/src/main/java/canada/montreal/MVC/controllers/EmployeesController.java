...
public class EmployeesController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			String[] oid = request.getRequestURI().substring(request.getContextPath().length()).split("\\;")[0]
					.split("/");
			logger.info("oid: {}", oid[2]);

			if ("register".equals(oid[2])) {
				logger.info("Enter register");
				employeeModel.handleRegister(request);
				RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/jsp/Registeremployees.jsp");
